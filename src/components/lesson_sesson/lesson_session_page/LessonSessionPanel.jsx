...
export const LessonSessionPanel = ({currentTask, handleNextTask, 
    ...
    <>
        <div style={{display:'flex', justifyContent:'center', gap: '15px'}}>
            {isRecording 
                ? <a onClick={()=> {stopRecording()}} className="btn defaultBtn" style={{width:'auto',padding:'0px 15px', backgroundColor:'red'}} >Стоп</a>
                : <a onClick={()=> {startRecording()}} className="btn defaultBtn" style={{width:'auto',padding:'0px 15px'}} >Ответить</a>}
            {currentTask.taskType !== 4 
                ? <a onClick={()=> {nextTask()}} className="btn defaultBtn" style={{width:'auto',padding:'0px 15px'}} >Далее</a>
                : <a onClick={()=> {endExamSession()}} className="btn defaultBtn" style={{width:'auto',padding:'0px 15px', backgroundColor:'green'}} >Завершить</a>}
